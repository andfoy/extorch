...
SortResult kthvalue(
    ...
    if(out_r.used) {
        ...
        std::tie(values_tensor, indices_tensor) = torch::kthvalue_out(
            values_tensor, indices_tensor, in_tensor, k, dim, keepdim);
    } else {
        std::tie(values_tensor, indices_tensor) = torch::kthvalue(
            in_tensor, k, dim, keepdim);
    }
