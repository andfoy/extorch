defmodule ExTorchTest.Export.ServerTest do
  use ExUnit.Case
  ...
  describe "start_link/1" do
    test "starts a server" do
      path = Path.join(@fixtures_dir, "simple_mlp_exported.pt2")
      {:ok, pid} = ExTorch.Export.Server.start_link(path: path)
    ...
    test "starts a named server" do
      path = Path.join(@fixtures_dir, "simple_mlp_exported.pt2")
      name = :"export_test_#{:erlang.unique_integer([:positive])}"
      {:ok, _} = ExTorch.Export.Server.start_link(path: path, name: name)
    ...
    test "handles concurrent predictions" do
      ...
      tasks =
        for _ <- 1..10 do
          Task.async(fn ->
            input = ExTorch.randn({1, 10})
            ExTorch.Export.Server.predict(pid, [input])
          end)
        end
  ...
  describe "info/1" do
    test "returns server info" do
      ...
      info = ExTorch.Export.Server.info(pid)
