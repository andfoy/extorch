defmodule ExTorchTest.Tensor.ComparisonTest do
  ...
  test "allclose/2" do
    t1 = ExTorch.tensor([10000.0, 1.0e-07])
    t2 = ExTorch.tensor([10000.1, 1.0e-08])
    assert !ExTorch.allclose(t1, t2)
  end

  test "allclose/3" do
    t1 = ExTorch.tensor([10000.0, 1.0e-08])
    t2 = ExTorch.tensor([10000.1, 1.0e-09])
    assert ExTorch.allclose(t1, t2, 1.0e-5)
  end

  test "allclose/3 with kwargs" do
    t1 = ExTorch.tensor([10000.0, 1.0e-08, :nan])
    t2 = ExTorch.tensor([10000.1, 1.0e-09, :nan])
